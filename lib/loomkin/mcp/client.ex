defmodule Loomkin.MCP.Client do
  @moduledoc """
  Connects to external MCP servers and makes their tools available to Loomkin.

  Each configured MCP server (in `.loomkin.toml`) gets an endpoint registered
  with jido_mcp's ClientPool. Tools discovered from those servers are converted
  into Jido.Action proxy modules and registered with `Loomkin.Tools.Registry`
