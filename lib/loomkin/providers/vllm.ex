defmodule Loomkin.Providers.Vllm do
  @moduledoc """
  ReqLLM provider for vLLM local LLM inference.
  ...
  - Base URL: `http://localhost:8000/v1` (configurable via `VLLM_HOST`)
  - Auth: No API key required (sends `"vllm"` as a dummy bearer token)
  - Model resolution: Builds synthetic `LLMDB.Model` structs since vLLM
    models aren't in LLMDB
  - Model discovery: Uses the OpenAI-compatible `/v1/models` endpoint

  ## Usage

      # After registration, use "vllm:model_name" as the model spec:
      Loomkin.LLM.stream_text("vllm:meta-llama/Llama-3.1-8B-Instruct", messages, opts)

  ## Configuration

      # Default: http://localhost:8000
      export VLLM_HOST=http://192.168.1.10:8000
  """
  ...
  @doc """
  Builds a synthetic LLMDB.Model struct for a vLLM model.

  Since vLLM models aren't cataloged in LLMDB, we construct a minimal
  ...
  def prepare_request(operation, _model_spec, _input, _opts) do
    {:error,
     ReqLLM.Error.Invalid.Parameter.exception(
       parameter: "operation: #{inspect(operation)} not supported by vLLM provider"
     )}
  end
  ...
  def attach(request, model, user_opts) do
    ...
    |> Req.Request.put_header("authorization", "Bearer vllm")
  ...
  def attach_stream(model, context, opts, _finch_name) do
    ...
    cleaned_opts =
      ...
      |> Keyword.put(:base_url, vllm_base_url())
    ...
    error ->
      {:error,
       ReqLLM.Error.API.Request.exception(
         reason: "Failed to build vLLM stream request: #{inspect(error)}"
       )}
  end
  ...
  @doc """
  Returns the configured vLLM base URL (OpenAI-compat endpoint).
  """
  def vllm_base_url do
    host = System.get_env("VLLM_HOST") || "http://localhost:8000"
  ...
  @doc """
  Fetches the list of models served by the vLLM instance.

  Calls the OpenAI-compatible `/v1/models` endpoint.
  ...
  def list_models do
    ...
    case Req.get(url, receive_timeout: 5_000, retry: false) do
      ...
      {:ok, %Req.Response{status: status}} ->
        {:error, "vLLM returned HTTP #{status}"}
  ...
  @doc """
  Returns true if vLLM is reachable.
  """
  def available? do
    case list_models() do
      {:ok, _} -> true
  ...
  defp resolve_model(model_spec) when is_binary(model_spec) do
    model_id = String.replace_prefix(model_spec, "vllm:", "")
