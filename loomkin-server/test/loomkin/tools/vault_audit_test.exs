defmodule Loomkin.Tools.VaultAuditTest do
  ...
  setup do
    vault_id = "vault-audit-test-#{System.unique_integer([:positive])}"
  ...
  describe "links check" do
    test "detects broken links", %{vault_id: vault_id} do
      # Create an entry
      Vault.write(
        vault_id,
        "notes/source.md",
    ...
    test "detects orphan entries", %{vault_id: vault_id} do
      Vault.write(
        vault_id,
        "notes/orphan.md",
        "---\ntitle: Orphan\ntype: note\n---\nNo one links here"
  ...
  describe "temporal check" do
    test "detects temporal language in evergreen entries", %{vault_id: vault_id} do
      Vault.write(
        vault_id,
        "notes/evergreen.md",
    ...
    test "ignores non-evergreen entries", %{vault_id: vault_id} do
      Vault.write(
        vault_id,
        "meetings/standup.md",
  ...
  describe "frontmatter check" do
    test "reports missing frontmatter fields", %{vault_id: vault_id} do
    ...
    test "passes when all required fields present", %{vault_id: vault_id} do
      %VaultEntry{}
      |> VaultEntry.changeset(%{
        vault_id: vault_id,
  ...
  describe "structure check" do
    test "detects type-directory mismatch", %{vault_id: vault_id} do
      %VaultEntry{}
      |> VaultEntry.changeset(%{
        vault_id: vault_id,
  ...
  describe "full audit" do
    test "returns clean report for healthy vault", %{vault_id: vault_id} do
      assert {:ok, %{result: result}} =
    ...
    test "runs all checks in full scope", %{vault_id: vault_id} do
  ...
  describe "error handling" do
    test "returns error for unknown scope", %{vault_id: vault_id} do
      assert {:error, msg} =
